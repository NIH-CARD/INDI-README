---
title: "hiPSCi cell line evaluation (hNGN2) - 01 Cell Calling"
...
lapply(sce_object, function(x){
  mat = assay(x)
  targets = mat[, Matrix::colSums(mat)!=0 & Matrix::colSums(mat) < 200]
...
For the hNGN2 sample, 100 is an acceptable threshold to use for the last 3 samples, but it is not sufficient for the first sample due to the higher sequencing depth of the first sample. As such, we instead use the 150 threshold, which will should be sufficient for all samples. 
...
set.seed(100)
lower_bound = 150
emptyDrops_out = lapply(1:length(sce_object), function(i) {
  x = sce_object[[i]]
