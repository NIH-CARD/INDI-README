---
title: "hiPSCi cell line evaluation (hNIL) - 07 Marker genes visualisation"
...
write.csv(cluster_markers_top5, file = "hNIL_differentiation_cluster_markers_any_top5.csv", row.names = FALSE)
```
...
write.csv(cluster_markers_all_sig, file = "hNIL_differentiation_cluster_markers_all_sig.csv", row.names = FALSE)
```
...
hnil_markers = c("NKX6-1", "PAX6", "OLIG2", "MNX1", "ISL2", "LHX4", "SLC18A3", "CHAT", "ONECUT1", "ONECUT2", "ONECUT3", "EBF1", "EBF2", "EBF3", "EBF4")
curated_markers = rbind(data.frame(gene=neuronal_markers, marker_type="neuronal"),
                        ...
                        data.frame(gene=hnil_markers, marker_type="hnil"))
```

Lirong has also shared with us the markers she normally use for annotating brain cells, which seems to have come from PanglaoDB.

```{r}
panglao_markers = read.csv("../panglao_brain_markers_dataframe.csv", header=F, stringsAsFactors = F, col.names = c("marker_type", "gene"))
```
...
for (marker_name in unique(curated_markers$marker_type)) {
  ...
  ggsave(paste0("hNIL_differentiation_",marker_name,"_curated_markers_expression_violin.png"), plot = plot_grid(plotlist = per_cluster_plots, ncol=1), width = 1*length(current_marker), height = 3*max(as.integer(sce_object_mnn$cluster)), units="cm", limitsize = FALSE)
}
```

```{r}
for (marker_name in unique(panglao_markers$marker_type)) {
  current_marker = intersect(panglao_markers[panglao_markers$marker_type == marker_name, c("gene")], rownames(merged_sce_object))
  per_cluster_plots = lapply(1:max(as.integer(merged_sce_object$cluster)), function(i) {
    plotExpression(merged_sce_object[, merged_sce_object$cluster == i], current_marker, point_size=0.5, theme_size=8) + ggtitle(paste0("Cluster ",i))
  })
  
  ggsave(paste0("hNIL_differentiation_",gsub("/", "-", gsub(" ", "_", marker_name)),"_panglao_markers_expression_violin.png"), plot = plot_grid(plotlist = per_cluster_plots, ncol=1), width = 1*length(current_marker), height = 3*max(as.integer(sce_object_mnn$cluster)), units="cm", limitsize = FALSE)
}
```
