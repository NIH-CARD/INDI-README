---
title: "hiPSCi cell line evaluation (cortical) - 07 Marker genes visualisation"
...
write.csv(cluster_markers_top5, file = "cortical_differentiation_cluster_markers_any_top5.csv", row.names = FALSE)
```
...
write.csv(cluster_markers_all_sig, file = "cortical_differentiation_cluster_markers_all_sig.csv", row.names = FALSE)
```
...
for (marker_name in unique(curated_markers$marker_type)) {
  ...
  ggsave(paste0("cortical_differentiation_",marker_name,"_curated_markers_expression_violin.png"), plot = plot_grid(plotlist = per_cluster_plots, ncol=1), width = 1*length(current_marker), height = 3*max(as.integer(sce_object_mnn$cluster)), units="cm", limitsize = FALSE)
}
```
