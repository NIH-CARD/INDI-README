---
title: "hiPSCi cell line evaluation (cortical) - 04 Doublet detection"
...
doublet_call = lapply(1:length(sce_object), function(i) {
  non_doublet_sce_object = sce_object[[i]][, sce_object[[i]]$vireo_assignment != "doublet" & sce_object[[i]]$multiseq_assignment != "Doublet"]
...
trend_plotlist = lapply(1:length(dec_object), function(i) {
  ...
  ggplot(data = data.frame(mean = fit$mean, var = fit$var, name=ifelse(fit$mean > 3 | fit$var > 2, names(fit$mean), NA)),
...
neighbourhood_doublet_call = lapply(1:length(sce_object), function(i){
  ...
  data.frame(doublet_proportion=doublet_neighbour_proportion, 
             neighbourhood_doublet_call=doublet_neighbour_proportion > (table(doublet_call[[i]])[["TRUE"]]/dim(sce_object[[i]][, sce_object[[i]]$vireo_assignment != "doublet" & sce_object[[i]]$multiseq_assignment != "Doublet"])[[2]]), 
...
sample_clusters_summary$n_multiseq_doublet_cells = sapply(1:nrow(sample_clusters_summary), function(row){
    sample = sample_clusters_summary$sample[row]
    subcluster = sample_clusters_summary$cluster[row]
    sum(colData(sce_object[[sample]])$multiseq_assignment[subclusters[[sample]] == subcluster] == "Doublet")
})

sample_clusters_summary$frac_multiseq_doublet_cells = sapply(1:nrow(sample_clusters_summary), function(row){
     sample_clusters_summary$n_multiseq_doublet_cells[row]/sample_clusters_summary$n_cells[row]
})
...
sample_clusters_summary$multiseq_doublet_outlier = sample_clusters_summary$frac_multiseq_doublet_cells > 0.5 #scater::isOutlier(sample_clusters_summary$frac_multiseq_doublet_cells, nmad=6, type="higher", log=FALSE)
...
noquote("MULTIseq doublet fraction outlier")
table(sample_clusters_summary$multiseq_doublet_outlier)
```
...
Let's also look at the agreement between the doublet score-based outlier and the multiseq based outlier

```{r}
table(sample_clusters_summary$hybrid2_doublet_outlier, sample_clusters_summary$multiseq_doublet_outlier, dnn=c("hybrid2 doublet fraction", "Multiseq doublet fraction outlier"))
```

Let's finally look at the agreement between the vireo based outlier and the multiseq based outlier

```{r}
table(sample_clusters_summary$vireo_doublet_outlier, sample_clusters_summary$multiseq_doublet_outlier, dnn=c("Vireo doublet fraction outlier", "Multiseq doublet fraction outlier"))
```
...
ggplot(data.frame(cluster = stringi::stri_join(sample_clusters_summary$sample, sample_clusters_summary$cluster), frac_multiseq_doublet_cells=sample_clusters_summary$frac_multiseq_doublet_cells, doublet=sample_clusters_summary$multiseq_doublet_outlier),
       aes(x=cluster, y=frac_multiseq_doublet_cells, fill=doublet)) +
  geom_bar(stat="identity") +
  labs(x = "Sample", y = "Fraction of multiseq doublet cell") +
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank())
```
...
ggplot(data.frame(cluster = stringi::stri_join(sample_clusters_summary$sample, sample_clusters_summary$cluster), frac_multiseq_doublet_cells=sample_clusters_summary$frac_multiseq_doublet_cells, doublet=sample_clusters_summary$multiseq_doublet_outlier),
       aes(x=cluster, y=frac_multiseq_doublet_cells, colour=doublet)) +
  geom_jitter() +
  labs(x = "Sample", y = "Fraction of multiseq doublet cell") +
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank())
```

Now to finalise the per sample doublet call. We will determine final doublet status based on if it's called as doublet by either median outlier or vireo/multiseq fraction. We will also ignore very small clusters too.

```{r}
sample_clusters_summary$is_doublet = (sample_clusters_summary$hybrid2_doublet_outlier | sample_clusters_summary$vireo_doublet_outlier | sample_clusters_summary$multiseq_doublet_outlier) & sample_clusters_summary$frac_cells <= 0.05
...
cell_summary$is_multiseq_doublet = Reduce(c, lapply(1:length(sce_object), function(i) {
    ifelse(sce_object[[i]]$multiseq_assignment == "Doublet", "Doublet", "Singlet")
  })
)
cell_summary$is_doublet = ifelse(cell_summary$is_cluster_doublet == "Doublet" | cell_summary$is_hybrid2_doublet == "Doublet" | cell_summary$is_vireo_doublet == "Doublet" | cell_summary$is_multiseq_doublet == "Doublet", "Doublet", "Singlet")
...
sce_object_mnn = correctExperiments(merged_sce_object, 
                                    ...
                                    PARAM=FastMnnParam(
                                      merge.order=as.list(intersect(list("Sample_SIGAE1", "Sample_SIGAF1"), merged_sce_object$Sample))
                                    ))
```
...
merged_subclusters = quickSubCluster(merged_sce_object, groups = merged_clusters, 
                                       prepFUN = function(x) {
                                         ...
                                         input = correctExperiments(x, 
                                                                    ...
                                                                    PARAM=FastMnnParam(
                                                                      merge.order=as.list(intersect(list("Sample_SIGAE1", "Sample_SIGAF1"), x$Sample))
                                                                    ))
                                       }, 
                                       clusterFUN = function(x) {
