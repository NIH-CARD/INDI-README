---
title: "hiPSCi cell line evaluation (iPSC) - 02 Quality Control Analysis"
...
For the undifferentiated iPSC, we don't see much difference in the ribosomal percentage across the different cell lines.
...
There seems to be roughly equal amount of UMI overall, which makes sense given that the cells are undifferentiated and therefore should practically be all 'identical'.
...
We do identify some outlier for ribosomal, but this is likely also outliers in other aspect as well (e.g. due to lib size of number or expressed genes)
