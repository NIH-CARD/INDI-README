---
title: "hiPSCi cell line evaluation (iPSC) - 04 Doublet detection"
...
plot_grid(plotlist = trend_plotlist, ncol=1)
```
...
neighbourhood_doublet_call = lapply(1:length(sce_object), function(i){
  knn_neighbours = BiocNeighbors::findKNN(reducedDim(sce_object[[i]], "PCA"), k=3)
...
lapply(1:length(sce_object), function(i){
  ...
  hybrid_score_plot = ggplot(as.data.frame(coord), aes(x = V1, y = V2, colour = hybrid_call)) +
    ...
    scale_colour_manual(values = c("#c6c6c6", "#ef0000")) +
...
Since the iPSC differentiation consist of only 1 sample, we will not do any cross sample doublet detection. We finalise the doublet call using the per sample doublet call

```{r}
doublet_calling_summary = cell_summary[-c(1)]

doublet_calling_summary$final_doublet_call = ifelse(doublet_calling_summary$is_doublet == "Doublet", "Doublet", "Singlet")
...
```{r}
table(sample=merged_sce_object$Sample, status=doublet_calling_summary$final_doublet_call)
```

```{r, fig.height=1.5, fig.width=3}
ggplot(data=melt(table(sample=merged_sce_object$Sample, status=doublet_calling_summary$final_doublet_call)), aes(x=sample, y=value, fill=status)) +
  geom_bar(stat="identity") +
  scale_x_discrete() +
  labs(x=NULL)+
...
```{r, fig.width=4}
ggplot(as.data.frame(umap_coord[[1]]), aes(x = V1, y = V2)) +
  geom_point(color="grey80", size = 0.5) +
  geom_point(data=as.data.frame(umap_coord[[1]])[doublet_calling_summary$final_doublet_call == "Doublet", c("V1", "V2")], color="darkred", size=0.7) +
  theme_classic() +
  theme(legend.position = "none") +
